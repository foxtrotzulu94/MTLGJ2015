    ...
    public void Ignite()
    {
        ...
        if (!IsDestroyedOnIgnition)
        {
            if (m_State == FlammableState.NotOnFire)
            {
                ...
                fireObject.transform.position = transform.position + Vector3.back * 0.1f;// +new Vector3(UnityEngine.Random.Range(-1.0f, 1.0f), UnityEngine.Random.Range(-1.0f, 1.0f), 0.0f) * 0.1f;
